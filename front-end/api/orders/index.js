(function () {
    ...
    app.post("/checkout", function (req, res, next) {
        ...
        request(endpoints.cartsUrl + "/cart/" + custId + "/order/done",
            function (error, response, body) {
                if (error) {
                    return next(error);
                }
                var bodyPost = {items: body, cId: custId};
                var options = {
                    uri: endpoints.ordersUrl +"/order/do",
                    method: 'POST',
                    json: true,
                    body: bodyPost
                };

                console.log("Save Order: " + JSON.stringify(options));
    ...
    app.get("/myorders", function (req, res, next) {
        ...
        request(endpoints.ordersUrl + "/orders/" + custId ,
            function (error, response, body) {
                if (error) {
                    return next(error);
                }
